...
| 7 | **Custom agents** | [`.kiro/agents/link-triage.json`](.kiro/agents/link-triage.json) + [`prompts/link-triage.md`](.kiro/agents/prompts/link-triage.md); [`.kiro/agents/check-builder.md`](.kiro/agents/check-builder.md) | `link-triage`: read-only agent that uses only the workspace `link-checkup` MCP server (`tools: ["@link-checkup"]`, `includeMcpJson: false` keeps other servers out) and pre-approves only `check_url` / `check_lookalike`; web, shell and write are excluded and denied by `permissions`, so it *cannot* open a link — it answers with a bilingual verdict + one concrete action, guided by its prompt file and the `triage-link` skill. `check-builder` (Markdown format): developer agent limited to `npm`/`git` commands and writes under `src/`, `test/`, docs, preloaded with steering + design. |
